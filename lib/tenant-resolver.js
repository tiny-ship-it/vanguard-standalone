...
function extractExternalId(context) {
  ...
  // HIERARCHY LEVEL 0: SSO/Corporate Identity
  // ================================================
  // If we have a corporate email or SSO identity, it takes precedence over Slack ID
  // to ensure cross-platform continuity (e.g., Slack, Web, CLI all use the same tenant).
  const ssoEmail = context.user?.email || context.user?.sso?.email;
  if (ssoEmail) {
    console.log(`[tenant-resolver] SSO context detected → Corporate tenant: corporate-${ssoEmail}`);
    return `corporate-${ssoEmail}`;
  }
 ...
 * Routing Logic:
 * ==============
 * 1. SSO/Corporate Identity (corporate-{email})
 * 2. If options.useSystem → SYSTEM_TENANT_UUID (manual override)
 * 3. DM context → Personal tenant (slack-{userId})
 * 4. Mapped channel → Channel tenant (channel-{purpose})
 * 5. Unmapped channel → Workspace tenant (workspace-{teamId})
 * 6. No context or resolution failure → null (no implicit system fallback)
 *
 * Example flows:
 * ==============
