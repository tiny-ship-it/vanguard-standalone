...
class Harness {
    ...
    async executeTask(message, sessionId, options = {}) {
        ...
        let result;
        try {
            result = await this.toolLoop.run(message, systemPrompt, { ...options, model });
            
            // Emit success event for supervisor
            this.emitActivity({
                ts: new Date().toISOString(),
                agentId: options.role || 'LEAD',
                sessionId,
                outcome: 'success',
                output: result,
            ...
            // Emit error event for supervisor
            this.emitActivity({
                ts: new Date().toISOString(),
                agentId: options.role || 'LEAD',
                sessionId,
    ...
    /**
     * Log activity for the Hermes Supervisor
     */
    emitActivity(event) {
        const fs = require('fs');
        const path = require('path');
        const logPath = path.join(__dirname, '../../memory/activity.jsonl');
        
        try {
            fs.mkdirSync(path.dirname(logPath), { recursive: true });
            fs.appendFileSync(logPath, JSON.stringify(event) + '\n');
        } catch (err) {
            console.error(`[vanguard-harness] Failed to log activity: ${err.message}`);
        }
    }
