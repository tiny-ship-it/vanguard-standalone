/**
 * Vanguard Namespace Manager (Formerly Assistant Core)
 ...
 * Implements Identity-Aware Proxying (IAP) logic to inherit user corporate permissions.
 */
async function checkVanguardPermissions(userId, resourceId, tenantId) {
    console.log(`[vanguard-security] Verifying permission for User: ${userId} on Resource: ${resourceId} in Tenant: ${tenantId}`);
    
    // In a real Vanguard deployment, this would validate the user's OIDC/SAML token 
    // against the enterprise identity provider (IdP) for the specified resource.
    
    // Local Permission Mapping (Bootstrapping phase)
    // For now, we assume all users in a tenant have access to the tenant's global/project namespace.
    // Specific file-level or tool-level permissions are checked here.
    
    // Example: Block access to protected memory files if user doesn't have the 'admin' scope
    if (resourceId.includes('/protect/') || resourceId.includes('SHIELD.md')) {
        // Mock permission check: Only allow specific users or those with elevated tokens
        const privilegedUsers = ['U09FF5JBXFW', 'U040AK20F54', 'U0PJ3T8LX'];
